...
## 套用語法規則 {#section_A7216BD585FF4EB88737B643B36C2781}
...
1. 對於可以選擇一個或多個字串序列的定義，請將字串視為一個集，其中重複條目被視為單個條目。 例如， `["foo", "bar", "foo", "baz"]` 等同於 `["foo", "bar", "baz"]`

1. 在定義數字時，規則之間會使用空格(例如 `Digit Digits`)，但套用規則時不應使用此類空格。

   例如，如果我們表示每個NonZeroInteger規則的數字123 *，則應將其表示為* ，而不是 `123``1 2 3`，即使規則包含NonZeroDigit和Digits之間的空格。

1. 有些規則允許多個表單。 在這些情況下，不同的表格會由字元分 `'|'` 隔。
   ...
   表示例項 `Foo` 可以取代為&quot;A&quot;、&quot;B&quot;或&quot;C&quot;。 這不應與跨越多行的表格混淆；這項功能可讓更長的表格更容易閱讀。
...
Sample *Output Protection Configuration(示例輸出保護配置* )主題給出了一個有效的配置及其語義。 本主題的上一 *節* ，介紹了配置的語法規則。 雖然語法有助於確保句法正確性，但有一些語法上合法的配置在語義上不正確（即，它們不是邏輯配置）。 本節介紹的配置在語 *法上* ，但語 *義上不正確* 。 請記住，本節中的示例已簡化為說明所討論方案所需的最小結構。
