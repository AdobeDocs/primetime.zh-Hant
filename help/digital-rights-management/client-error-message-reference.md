...
| 3310 | AppIDMismatch | 應用程式未列在白名單中。 Android、iOS或Flash SWF。<br>subErrorId:郵遞區號1000942;播放受保護的串流時出錯。 FAXS錯誤。<br>此外，用戶端也可能報告pubID（應用程式發行者ID）的空字串。<br>**Android:**Android應用程式與使用中的應用程式不符。 請記住，Android中除錯密鑰庫的目錄通常與發行密鑰庫的目錄不同。<br>**iOS:** 請參閱TVSDK iOS指南中的「Whitelist your iOS applicationdocumentation」（iOS應用程式白名單）。 |
