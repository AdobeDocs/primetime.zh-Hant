...
- *softwarestatement*：從TVE控制面板或取得的值 *null* 若在strings.xml中設定&quot;software\_statement&quot;
- *redirectUrl* ：對於FireTV實施，此引數應該為Null。 此屬性上的任何設定都將被忽略。
...
如果 *url* 不會使用引數，因此產生的網路呼叫會鎖定預設服務提供者URL： Adobe Release Production環境。
...
- f.登出：
   ...
   - SDK將會開啟Chrome自訂分頁以存取mvpd\_id登出端點。 完成後，Chrome自訂標籤將關閉
