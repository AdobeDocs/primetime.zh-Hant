...
實際的服務提供者(SP)角色由程式設計師擔任，但Adobe Primetime驗證可作為該程式設計師的SP Proxy。 使用Adobe Primetime驗證作為中介，可讓MVPD和程式設計師避免根據個別情況自訂其權益程式。

當程式設計師從支援SAML的MVPD要求驗證時，以下步驟會使用Adobe Primetime驗證呈現事件的序列。 請注意，Adobe Primetime驗證存取啟用程式元件在使用者/訂閱者的使用者端上處於作用中狀態。 Access Enabler從此協助進行驗證流程的所有步驟。
...
此 [線上內容存取(OLCA)驗證與授權介面規格](https://www.cablelabs.com/specifications/search?query=&amp;category=&amp;subcat=&amp;doctype=&amp;content=false&amp;archives=false){target=_blanck} 提供標準AuthN請求和回應。 雖然Adobe Primetime驗證不要求MVPD將其權益訊息設定在此標準上，但檢視規格可提供AuthN交易所需關鍵屬性的深入分析。

>[!NOTE]
>
>MVPD透過Adobe Primetime驗證接收的AuthN要求包含數位簽名。 不過，為了簡短起見，下列範例不會顯示簽章。 如需顯示數位簽章的範例，請參閱中的範例： [驗證回應](#authn-response) ，請於下列章節中說明。
...
| ID | 要求的識別碼。 另請參閱 [SAML core 2.0-os](http://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf){target=_blank} 第1.3.4節以取得詳細資料。 |
| IsPassive | Boolean值。 如果為「true」，則身分提供者和使用者代理程式本身「不得」明顯從請求者取得使用者介面的控制權，並以明顯的方式與簡報者互動。 如果未提供值，預設值為「false」。 |
| Isseminstant | 回應問題時的即時時間。 時間值會以UTC編碼，如所述 [SAML core 2.0-os](http://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf){target=_blank} 區段1.3.3。 |
| ProtocolBinding | URI參照，識別傳回 &lt;response> 訊息。 另請參閱 [SAMLBind] 以取得有關協定繫結及為其定義的URI參照的詳細資訊。 預設值： urn:oasis:名稱:tc:SAML：2.0:bindings:HTTPPOST |
| 版本 | 此請求的版本。 |
...
| 格式 | 指定與名稱識別碼格式對應的URI參照Default： urn:oasis:名稱:tc:SAML：2.0:nameid-format:暫時Adobe建議： urn:oasis:名稱:tc:SAML：2.0:nameid-format:持續 |
