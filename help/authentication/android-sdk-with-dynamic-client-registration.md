---
title: 具有動態用戶端註冊的Android SDK
description: 具有動態用戶端註冊的Android SDK
...
# 具有動態用戶端註冊的Android SDK {#android-sdk-with-dynamic-client-registration}
...
Android適用的Android AccessEnabler SDK已修改為啟用驗證，而不使用工作階段Cookie。 隨著越來越多的瀏覽器限制對Cookie的存取，需使用其他方法來允許驗證。

若為Android，使用Chrome自訂分頁會限制從其他應用程式存取Cookie。

>**Android SDK 3.0.0** 介紹：

- 動態用戶端註冊會根據簽署的請求者ID和工作階段cookie驗證，取代目前的應用程式註冊機制
- 驗證流程的Chrome自訂分頁

>[!NOTE]
>
>若為不支援Chrome自訂分頁的舊版Android，則會使用與舊版AccessEnabler SDK類似的WebView驗證。


## 動態客戶端註冊 {#DCR}

Android SDK v3.0+將使用動態用戶端註冊程式，如 [動態客戶端註冊](/help/authentication/dynamic-client-registration.md).


## 功能示範 {#Demo}

請看 [本網路研討會](https://my.adobeconnect.com/pzkp8ujrigg1/) 此功能提供功能的更多內容，並包含如何使用TVE控制面板管理軟體陳述式，以及如何使用Adobe隨Android SDK提供的示範應用程式來測試產生的陳述式。
...
- redirectUrl :唯一url，即TVE控制面板或 *null* 如果在字串中設定&quot;redirect\_uri&quot;

注意：無效的softwareStatement或redirectUrl會導致應用程式無法初始化AccessEnabler或註冊應用程式以進行Adobe Pass驗證和授權
</br>
注意：redirectUrl參數或strings.xml中的redirect\_uri應是在TVE儀表板中為應用程式添加的反向順序的域的值(例如：若為「TVE控制面板」中新增的網域「adobe.com」，redirectUrl應為「com.adobe」。
 
...
- *url*:選用參數；依預設，會使用Adobe服務提供者 [http://sp.auth.adobe.com/](http://sp.auth.adobe.com/). 此陣列可讓您為Adobe提供的驗證和授權服務指定端點（可能會使用不同的執行個體進行除錯）。 您可以使用此ID來指定多個Adobe Primetime驗證服務提供者例項。 執行此操作時，MVPD清單由來自所有服務提供者的端點組成。 每個MVPD都與最快的服務提供商相關聯；即第一個回應且支援該MVPD的提供者。
...
a.從Adobe Pass（TVE儀表板）獲取software\_statement和redirect\_uri
...
- 註冊申請：使用 **軟體\_語句**,SDK將取得 **client\_id, client\_secret, client\_id\_issued\_at, redirect\_uris, grant\_types**. 此資訊將儲存在應用程式的內部儲存中。
...
| HTTP 400(Bad Request) | {&quot;error&quot;:&quot;無效\_client&quot;} | 客戶端身份驗證失敗，因為客戶端未知。 SDK必須重新向授權伺服器註冊。 |
| HTTP 400(Bad Request) | {&quot;error&quot;:&quot;unauthorized\_client&quot;} | 已驗證的客戶端無權使用此授權授權類型。 |

- 如果MVPD需要被動驗證，Chrome自訂標籤會開啟以使用該MVPD執行被動，並會在完成時關閉
...
- mvpd已記得：前往setSelectedProvider(mvpd_id)
...
- 已取消登入：重置MVPD選擇
- URL配置會建立為「adobepass://redirect_uri」，以在驗證完成時擷取

e.get/checkAuthorization :SDK將包含 **access_token** 在標題中作為授權：承載 **access_token**
...
- SDK會開啟Chrome自訂分頁以到達mvpd_id登出端點。 完成後，將會關閉Chrome自訂分頁
- URL配置會建立為「adobepass://logout」，以擷取登出完成的時刻
- logout會觸發sendTrackingData(new Event(EVENT_LOGOUT,USER_NOT_AUTHENTICATED_ERROR)和回呼：setAuthenticationStatus(0,&quot;Logout&quot;)
