...
- 有關基本的黃金時段身份驗證權利流程的說明，請參閱 [權利流](https://tve.helpdocsonline.com/entitlement-flow)。
- 有關如何使用此API實現黃金時段身份驗證權利流的逐步介紹，請參見 [iOS整合烹飪書](/help/authentication/iostvos-sdk-cookbook.md)。
- 有關最新的iOSAccessEnabler SDK，請轉到 <https://tve.zendesk.com/hc/en-us/articles/204963209-iOS-Native-AccessEnabler-Library>。 

 

**注：** Adobe鼓勵您僅使用黃金時段身份驗證 *公共* API:
...
- [getAuthentication：篩選器](#getAuthN_filter)。[ ](#getAuthN_filter)[getAuthentication（獲取身份驗證）:withData:](#getAuthN)[和篩選器](#getAuthN_filter)  — 啟動完整身份驗證工作流。
...
- *選項*:包含全局SDK選項的NSDictionary。 目前，以下選項可用：
   ...
   - **句柄SVC**  — 指示程式設計師是否將處理SFSafariViewControllers的布爾值。 請參閱 [iOSSDK 3.2+上的SFSafariViewController支援](https://tve.helpdocsonline.com/sfsafariviewcontroller-support-on-ios-sdk-3.2) 的子菜單。
...
- *URL*:可選參數；預設情況下，使用Adobe服務提供程式(http://sp.auth.adobe.com/)。 此陣列允許您為Adobe提供的身份驗證和授權服務指定端點（可能會將不同的實例用於調試目的）。 您可以使用它來指定多個Moginiqe身份驗證服務提供程式實例。 執行此操作時，MVPD清單由來自所有服務提供商的端點組成。 每個MVPD都與最快速的服務提供商相關聯；即，首先響應的提供程式，並且支援該MVPD。
   ...
   **注：** 如果調用時沒有 `serviceProviders` 參數，庫將從預設服務提供程式(如 `https://sp.auth.adobe.com` 生產配置檔案，或登台配置檔案的https://sp.auth-staging.adobe.com)。 如果 `serviceProviders` 參數，它必須是URL的陣列。從所有指定端點檢索配置資訊並合併。 如果不同的服務提供商響應中存在重複資訊，則衝突被解決，有利於最快響應的伺服器（即優先於具有最短響應時間的伺服器）。
...
- *簽名請求者ID*: **此參數在iOSAccessEnabler 1.2及更高版本中存在。** 用您的私鑰進行數字簽名的請求者ID的副本。 有關詳細資訊，請參閱 [註冊本地客戶端](https://tve.helpdocsonline.com/registering-native-clients)。
- *URL*:可選參數；預設情況下，使用Adobe服務提供程式(http://sp.auth.adobe.com/)。 此陣列允許您為Adobe提供的身份驗證和授權服務指定端點（可能會將不同的實例用於調試目的）。 您可以使用它來指定多個Moginiqe身份驗證服務提供程式實例。 執行此操作時，MVPD清單由來自所有服務提供商的端點組成。 每個MVPD都與最快速的服務提供商相關聯；即，首先響應的提供程式，並且支援該MVPD。

**附註：** 如果調用時沒有 `serviceProviders` 參數，庫將從預設服務提供程式中檢索配置(如： `https://sp.auth.adobe.com` 用於生產配置檔案或 `https://sp.auth-staging.adobe.com` )。 如果 `serviceProviders` 參數，它必須是URL的陣列。從所有指定端點檢索配置資訊並合併。 如果不同的服務提供商響應中存在重複資訊，則衝突被解決，有利於最快響應的伺服器（即優先於具有最短響應時間的伺服器）。
...
最後， AccessEnabler將調用 [`setAuthenticationStatus()`](http://tve.helpdocsonline.com/ios-tvos-api-reference$setAuthNStatus) 回調，狀態代碼為0，表示註銷流成功。
...
- *鍵字典*:字典資料結構，格式如下：
   ...
   **注：** 程式設計師可用的實際用戶元資料取決於MVPD提供的內容。  此清單將隨著新的元資料被提供並添加到黃金時段身份驗證系統中而擴展。

**觸發的回調：** [`setMetadataStatus:encrypted:forKey:andArguments:`](#setMetaStatus)

**更多資訊：** [用戶元資料](https://tve.helpdocsonline.com/user-metadata-v2)
...
**更多資訊：** [iOS/電視OS單一登錄](https://tve.helpdocsonline.com/ios-tvos-sdk-api-reference#)
...
- (BOOL)enablePlatformServices — 如果為true，則MVPD支援SSO服務，如 [Apple](https://tve.helpdocsonline.com/ios-tvos-sdk-api-reference#)。
...
- [權利流](https://tve.helpdocsonline.com/entitlement-flow)
- [黃金時段驗證中的跟蹤資料](https://tve.helpdocsonline.com/tracking-data-in-adobe-pass)
