...
廣告追蹤不由TVSDK內部處理，例如Adobe Primetime廣告決策（先前稱為Auditude）解決廣告的時間。 不過，TVSDK提供下列抽象化，以定義廣告相關內容在時間軸上的呈現方式：
...
TVSDK會將廣告追蹤事件派單到您的應用程式，因此您可以實作自己的追蹤邏輯。 如果您設定自訂廣告標籤，則會收到`AD_BREAK_STARTED`、`AD_STARTED`、`AD_PROGRESS`、`AD_COMPLETED`和`AD_BREAK_COMPLETED`事件。