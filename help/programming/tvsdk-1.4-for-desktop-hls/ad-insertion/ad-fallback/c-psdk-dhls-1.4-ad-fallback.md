...
1. 設定 `fallbackOnInvalidCreative` 設為true，則當線性/內嵌廣告的媒體型別對HLS無效時，可讓VMAP後退。
...
在TVSDK中，唯一有效的媒體型別為 `application/x-shockwave-flash` (VPAID)和 `application/x-mpegURL` (m3u8)。
