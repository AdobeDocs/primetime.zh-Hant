...
Primetime廣告插入後端會維護一組優先順序，讓它能夠在相同的VAST/VMAP回應中，選擇不同的媒體類型。 您可在CRS概觀中進一步瞭解此優先順序清單，以及如何 [變更清單](../../../../dynamic-ad-insertion/creative-repackaging-service/crs-overview.md)。

## 定義VMAP內嵌廣告的備援廣告行為 {#define-fallback-ad-behavior-for-vmap-inline-ads}

當VMAP內嵌廣告包含無效的媒體類型時，您可以開啟備援。

1. 當線 `fallbackOnInvalidCreative` 性／內嵌廣告的媒體類型對HLS無效時，設為true可讓VMAP回落。
...
在TVSDK中，唯一有效的媒體類 `application/x-shockwave-flash` 型是(VPAID) `application/x-mpegURL` 和(m3u8)。
