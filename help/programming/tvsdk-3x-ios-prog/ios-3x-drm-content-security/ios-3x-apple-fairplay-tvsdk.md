...
## 在TVSDK應用程式中啟用Apple公平播放 {#section_61CFA3C22FE64F52B2C8CE860B72E88B}

您可以在TVSDK應用程式中實施Apple的DRM解決方案FairPlay Streaming。

1. 通過實施FairPlay客戶資源載入器建立 `PTAVAssetResourceLoaderDelegate`。 有關詳細資訊，請參閱TVSDK應用程式中的AppleFairPlay。
   ...
   方法 `resourceLoader:shouldWaitForLoadingOfRequestedResource` 等於 `AVAssetResourceLoaderDelegate`。
