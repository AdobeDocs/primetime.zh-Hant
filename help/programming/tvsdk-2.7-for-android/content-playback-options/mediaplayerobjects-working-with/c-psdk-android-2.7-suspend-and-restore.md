---
description: 裝置熒幕關閉和開啟時，您必須由應用程式處理，才能暫停及還原TVSDK MediaPlayer。
...
TVSDK無法判斷片段（或活動）何時在背景或前景。 此外，Android `SurfaceView` 裝置畫面關閉時不會遭到破壞（但活動已暫停）。 不過， `SurfaceView` *會* 當裝置將您的應用程式置於背景時，就會遭到破壞。 TVSDK無法偵測到任何這些變更，因此應用程式必須處理這些變更。
