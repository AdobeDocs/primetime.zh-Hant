...
* HLS第4版功能
   ...
   * **特技播** 放模式支援使用I-Frames的HLS串流的快進和倒退。所有視訊播放控制項都可在內容上運作。 慢動作（前向）適用於速率介於0和1之間的外部視訊播放模式。
   * **可調式位元速率(ABR)** 可讓播放器根據網路和其他條件，動態選擇要播放相同內容串流的多個版本。您可以動態設定參數或在資訊清單檔案中設定參數，以在積極、中和和保守的選擇原則中進行選擇。
   * **位元組** 範圍允許單個TS檔案包含多個TS段。
   * **替代音** 訊轉譯可讓播放器在可用音軌之間切換。
...
* 廣告／替代內容與獲利功能
   ...
   * **一致的廣告行為。** 使用介 `AdPolicySelector` 面可讓所有播放器的行為一致，例如在內容中出現廣告時進行搜尋和特技播放。如果您未實作自己的TVSDK，則TVSDK會使用`DefaultAdPolicySelector`。
   * **移除／取代C3廣告。** 使用適當的TVSDK API來移除自訂內容範圍，並動態插入新廣告，毋需額外的準備工作。當廣播即時／線性內容後，即時隨選提供而不需進行清理時，此功能就十分方便。

以下是2.4版的主要新功能：

* **VOD和即時的立即開** 啟當您啟用即時開啟時，TVSDK會在播放開始前初始化媒體並緩衝媒體。由於您可以在背景中同時啟動多個`MediaPlayerItemLoader`實例，因此可以緩衝多個流。 當使用者變更頻道，而串流已正確緩衝時，新頻道上的播放會立即開始。 TVSDK 2.4也支援即時串流的Instant On。 即時視窗移動時，即時串流會重新緩衝。

* **效能改進**全新的TVSDK 2.4架構提供多種效能改進：

   * **子分段** - TVSDK進一步縮小每個片段的大小，以盡快開始播放。
