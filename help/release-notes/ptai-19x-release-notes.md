...
* CRS:新增從特定廣告系統、廣告ID、創意ID、創意URL及／或創意格式將CRS請求列入黑名單／列入白名單的能力

   >注意
   >
   >當找到相同廣告的具有動態值（例如，URL中的動態參數）的廣告提供者時，Adobe會新增黑名單規則。 在動態元件解決後，此類黑名單規則將會停用，不論是由提供者解決或透過標準化規則。

   * 如果您想要為您的區域添加黑名單或白名單規則，請與技術客戶經理聯繫以獲得幫助。
