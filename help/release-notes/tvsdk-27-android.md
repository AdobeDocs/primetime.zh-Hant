...
# TVSDK 2.7 for Android版本注意事項 {#tvsdk-for-android-release-notes}
...
Android專用的TVSDK 2.7包含1.4版的所有功能，但「功能表」下不支援的 [功能除外](#feature-matrix)。
...
### 舊版的新功能 {#new-features-previous-releases}
...
* WebViewDebbugingWebViewDebbuging值的預設值的更改預設設定為False。 若要啟用它，請在應用程式中呼叫setWebContentsDebuggingEnabled(true)。
* OpenSSL和Curl版本升級將libcurl更新為v7.57.0，將OpenSSL更新為v1.0.2k。
* VAST回應物件的應用程式層級存取引進新的API NetworkAdInfo::getVastXml()，可讓您存取應用程式的VAST回應物件。
...
* [TVSDK Android C++ API檔案](https://help.adobe.com/en_US/primetime/api/psdk/cpp/namespaces.html) -每個Java類都有對應的C++類別，而C++檔案所包含的說明資料比Javadoc更多，請參閱C++檔案以進一步瞭解Java API。
