...
description: Android適用的TVSDK 3.15發行說明說明TVSDK Android 3.15中的新增或變更、已解決和已知問題及裝置問題
...
適用於Android的TVSDK 3.15發行說明說明TVSDK Android 3.15中的新增或變更、已解決和已知問題以及裝置問題。
...
>
>1. 從下載VideoHeartbeat.jar [https://github.com/Adobe-Marketing-Cloud/video-heartbeat-v2/releases](https://github.com/Adobe-Marketing-Cloud/video-heartbeat-v2/releases) （Android適用的VideoHeartbeat程式庫v2.0.0）
>1. 將VideoHeartbeat.jar解壓縮至libs/資料夾。


Android適用的TVSDK提供比舊版更多的效能改善。 除了Multi-CDN支援外，提供高品質的檢視體驗，並包含1.4版的所有功能。

支援和不支援的完整功能集顯示在 [功能矩陣](#feature-matrix) 一節。

## Android TVSDK 3.15

此版本修正創意標籤遺失或應用程式當機多次的問題 [!UICONTROL url CDATA] 在中為空 [!UICONTROL VAST] 回應。

若要了解此版本和舊版的錯誤修正，請參閱 [在Android適用的TVSDK中修正的問題](#resolved-issueszd).

### 舊版中的新功能和增強功能

**Android TVSDK 3.14**

此版本修正應用程式在 [!UICONTROL CDATA] 節點對於任何 [!UICONTROL ClickTracking], [!UICONTROL CustomClick] 或 [!UICONTROL CompanionClickTracking] VAST回應中的元素。
...
若要使用Android Studio來設定及執行參考應用程式，請遵循TVSDK zip所提供之ReadMe檔案的指示 `TVSDK_Android_x.x.x.x/samples/PrimetimeReference/src/README.md`.

在目前版本中修正的主要客戶問題，請參閱 [已解決的問題](#resolved-issues) 區段。

**Android TVSDK 3.11**

* **允許保護系統特定標題(PSSH)盒取用** - TVSDK允許擷取與目前載入的媒體資源相關聯的保護系統特定標題方塊。 新API `getPSSH()` 新增至 `com.adobe.mediacore.drm.DRMManager`.

如需詳細資訊，請參閱 [Widevine DRM](../programming/tvsdk-3x-android-prog/android-3x-content-security/android-3x-drm-widevine.md).

**Android TVSDK 3.10**

此版本著重於修正下列主題中提及的常見客戶問題： [已解決的問題](#resolved-issues) 區段。

**Android TVSDK 3.9**

* **透過HTTPS進行安全傳送** - Android TVSDK 3.9推出透過HTTPS的安全傳送功能，以無與倫比的規模和效能安全地傳送內容。

   為了透過HTTPS安全傳送，於 `NetworkConfiguration` 類別。
...
* 針對Widevine測試內容，新的API `setMediaDrmCallback` 在DRManager類中公開以覆蓋MediaDrmCallback介面的預設實施。

   `public static void setMediaDrmCallback(MediaDrmCallback callback)`

* 修正未處理的AppCrash錯誤 `MediaPlayerEvent.ITEM_UPDATED` 在C++層（Android 64位元）。

**Android TVSDK 3.6**

* **根據64位元要求增強您的應用程式**  — 原生程式庫 `(libAVEAndroid.so)` 現已升級，並可在兩個版本中使用。 現有armeabi（32位）原生程式庫位置已從 `/framework/Player to /framework/Player/armeabi` 此外， `/framework/Player/arm64-v8a.`

**版本3.5**

* **及時的廣告解決** - TVSDK 3.5從時間軸移除對已播放廣告的支援。

* **啟用離線播放支援**  — 透過離線播放，使用者現在可以下載視訊內容至裝置，並在未連線時觀看。 有關詳細資訊，請參閱[使用Android的離線播放](https://helpx.adobe.com/content/dam/help/en/primetime/programming-guides/psdk_android_3.5.pdf).&quot;
...
* **API變更**
   ...
   * 新增API至 `NetworkConfiguration::setNumOfTimesManifestRetryBeforeError(n)*` 處理網路錯誤和逾時。
      * 其中(n)為重試次數。
...
* **使用AC3/EAC3音頻編解碼器啟用流的回放**
   ...
   * `void alwaysUseAC3OnSupportedDevices(boolean val)` in `MediaPlayer` 類
...
* **準時 — 解析靠近廣告標籤的廣告**
「延遲廣告解決」現在可以獨立解析每個廣告插播。 之前，廣告解決是分兩階段的方法：已在播放開始前解決前段，且在播放開始後結合所有中段/後段槽。 透過這項增強功能，每個廣告插播現在都會在廣告提示點之前的特定時間解決。

>[!NOTE]
>
>「延遲廣告解決」現在已變更為預設關閉，且需明確啟用。

新增API至 `AdvertisingMetadata::setDelayAdLoadingTolerance` 取得與此廣告中繼資料相關聯的延遲廣告載入容差。\
現在，搜尋可在PREPARATION後立即允許，搜尋廣告插播將會在搜尋完成前立即解決。\
信令模式 `SERVER_MAP` 和 `MANIFEST_CUES` 。

如需詳細資訊，請參閱 [Android適用的TVSDK 3.0程式設計師指南](../programming/tvsdk-3x-android-prog/android-3x-advertising/ad-insertion/c-lazy-ad-resolving/c-lazy-ad-resolving.md) 在API和事件變更上。

* **已更新 `targetSdkVersion` 至最新版本**

已更新 `targetSdkVersion` 從19到27。
...
* **啟用背景音訊**
   ...
   若要在應用程式從前景移至背景時啟用音訊播放，應用程式應呼叫 `enableAudioPlaybackInBackground` 播放器處於PREPARED狀態時，以true作為引數的MediaPlayer API。
...
* **移除NetworkConfiguration類別中的API setEncodeUrlForTracking** 因為URL中不安全的字元應進行編碼。
...
* 預設值的變更 `WebViewDebbuging`
   ...
   `WebViewDebbuging` 值設為 `Fals`預設為。 若要啟用，請呼叫 `setWebContentsDebuggingEnabled(true)` 中。
...
* VAST回應物件的應用程式層級存取
   ...
   導入新API `NetworkAdInfo::getVastXml()` 提供對應用程式的VAST響應對象的訪問。
...
* API變更：
   ...
   * 新事件 `CookiesUpdatedEvent` 中。 更新Cookie時，會由媒體播放器發送。

   * 新增API至 `NetworkConfiguration::set/ getCustomUserAgent()` 使用自訂使用者代理。

   * 新增API至 `NetworkConfiguration::set/ getEncodedUrlForTracking` 強制編碼不安全字元。

   * 新增API至 `NetworkConfiguration::getNetworkDownVerificationUrl()` 設定網路驗證URL，以防發生故障轉移。

   * 新增屬性至 `TextFormat::treatSpaceAsAlphaNum` 定義在顯示字幕時是否將空間視為英數字元。

* 中的變更 `SizeAvailableEvent`. 以前， `getHeight()` 和 `getWidth()` 方法 `SizeAvailableEvent` 在2.5.2中，用於傳回影格高度和影格寬度，並以媒體格式傳回。 現在，它分別傳回由解碼器傳回的輸出高度和輸出寬度。
...
* **即時開啟（適用於VOD和即時播放） —** 當您啟用即時啟用時，TVSDK會在播放開始前初始化及緩衝媒體。 因為您可以在背景中同時啟動多個MediaPlayerItemLoader例項，所以可以緩衝多個資料流。 當使用者變更頻道，且資料流已正確緩衝時，新頻道上的播放會立即開始。 TVSDK 2.5.1也支援的「立即開啟」 **live** 資料流。 即時視窗移動時，即時資料流會重新緩衝。

* **改善ABR邏輯 —** 新的ABR邏輯基於緩衝器長度、緩衝器長度變化率和測量頻寬。 這確保ABR在頻寬波動時選擇正確的位速率，並通過監視緩衝長度變化的速率來優化位速率交換機實際發生的次數。

* **部分區段下載/子區段 —** TVSDK會進一步縮小每個片段的大小，以便盡快開始播放。 ts片段必須每兩秒有一個關鍵幀。

* **延遲廣告解決 —** TVSDK不會等到非前段廣告的解析度再開始播放，因而縮短了啟動時間。 在所有廣告都解決之前，仍不允許使用搜尋和特技播放等API。 這適用於與CSAI搭配使用的VOD資料流。 在廣告解析完成前，不允許執行搜尋和快速前進等操作。 針對即時資料流，無法在即時事件期間為廣告解析啟用此功能。

* **持久網路連接 —** 此功能可讓TVSDK建立並儲存持續網路連線的內部清單。 這些連接可重複用於多個請求，而不是為每個網路請求開啟新連接，然後在之後銷毀它。 這可提高網路程式碼的效率並減少延遲，進而提高播放效能。
TVSDK開啟連線時，會向伺服器要求 *保存* 連線。 某些伺服器可能不支援此類型的連線，在此情況下，TVSDK會回復為每個要求重新建立連線。 此外，雖然永久連線預設會開啟，但TVSDK現在提供設定選項，讓應用程式可視需要關閉永久連線。

* **並行下載 —** 同時下載視訊和音訊而非串列下載可減少啟動延遲。 此功能可讓HLS Live和VOD檔案播放、從伺服器最佳化可用頻寬使用、降低在執行不充分的情況下進入緩衝區的可能性，以及將下載和播放之間的延遲降至最低。

* **平行廣告下載 —** TVSDK在點擊廣告插播前，會平行預先擷取廣告，以順暢播放廣告和內容。
...
* **使用自適應比特率(ABR)進行特技播放 —** 此功能可讓TVSDK在iFrame串流之間切換，同時處於特技播放模式。 您可以使用非iFrame描述檔以較低的速度進行特技播放。

* **更流暢的特技遊戲 —** 這些改善功能可增強使用者體驗：
...
* **內容保護**
   ...
   * **基於解析度的輸出保護 —** 此功能將播放限制系結至特定解析度，提供更精細的DRM控制。
...
* **工作流程支援**
   ...
   * **改進故障轉移支援 —** 已實作其他策略，即使主機伺服器、播放清單檔案和區段失敗，仍可繼續不間斷播放。


* **廣告**

   * **Moat整合 —** 支援來自Moat的廣告檢視度測量。

   * **隨附橫幅 —** 隨附橫幅會與線性廣告一併顯示，且通常會在廣告結束後繼續顯示在檢視上。 這些橫幅可以是html類型(HTML片段)或是iframe類型（iframe頁面的URL）。

* **Analytics**

   * **VHL 2.0 -** 這是最新最佳化的視訊心率程式庫(VHL)整合，可自動收集Adobe Analytics的使用資料。 API的複雜度已降低，以便輕鬆實作。 下載VHL程式庫 [Android適用的v2.0.0](https://github.com/Adobe-Marketing-Cloud/video-heartbeat-v2/releases) 並解壓縮libs資料夾中的JAR檔案。

* **SizeAvaliableEventListener**

   * `getHeight()` 和 `getWidth()` 方法 `SizeAvailableEvent` 現在會分別以高度和寬度傳回輸出。 顯示外觀比例的計算方式如下：
...
* **Cookie**
   ...
   * Android TVSDK現在支援存取儲存在Android應用程式CookieStore中的JAVA Cookie。 提供回呼API(onCookiesUpdated)，以在每次有新Cookie進入時進行記錄 **Set-Cookie** 回應標題。 使用CookieStore在該特定URI/網域上設定這些Cookie值，即可將這些Cookie作為用於不同URI/網域的HttpCookie清單使用。 同樣地，TVSDK中的Cookie值也會使用CookieStore新增API來更新。
...
* ZD#46903 — 當創意標籤遺失或 [!UICONTROL url CDATA] 在中為空 [!UICONTROL VAST] 回應。
...
* ZD #31533 — 在應用程式傳送至背景後，在Android上播放音訊。
   ...
   * 新增 `enableAudioPlaybackInBackground` 應以「True」作為引數呼叫的MediaPlayer API（當播放器處於「已準備」狀態時），以在應用程式於背景時啟用音訊播放。
...
* ZD #26614 — 緊急 — 第三方廣告服務/程式化 — 無法提供曝光數。
   ...
   * 增強先前的修正，方法是處理XML剖析中的案例，當「空間」在「等號」前面時，問題可重現，例如 &lt;vast version=&quot;2.0&quot;>
...
* ZD #32584 — 無法存取 &lt;extensions> VAST響應的節點。

   * 修正XML剖析的相關問題，現在NetworkAdInfo提供 &lt;extensions> 節點
...
* Zendesk #30727 VOD廣告未下載/解析。
   ...
   在VMAP XML中，如果有空的VAST標籤沒有明確的結尾標籤(「&lt;/vast>&#39;)後面沒有新行字元，則VMAP XML無法正確剖析，廣告可能無法播放。
...
* 對於廣告進度事件，時間軸列可能無法反映100%準確的廣告播放時間。 作為因應措施，您可使用 `adcompleteevent` 了解廣告播放完成和更新UI，以用於更新時間軸列、移除廣告相關UI等各種用途。
...
* 低端裝置上的即時視訊播放可能會發生音訊 — 視訊同步問題。
* 對於FER流，virtualTime和localTime可能不同。 此外，帶偏移的FER無法運作。
* 在VMAP XML中，如果有空的VAST標籤，且沒有明確的結尾標籤(&lt;/vast>)，若後面沒有新行，則VMAP XML無法正確剖析，且廣告可能無法播放。
* 不支援VPAID貼文。
...
* [TVSDK Android C++ API檔案](https://help.adobe.com/en_US/primetime/api/psdk/cpp_3.5/namespaces.html)  — 每個Java類都有相應的C++類，而C++文檔包含的解釋性資料比Javadoc更多，因此，請參閱C++文檔，以更深入地了解Java API。
* [Android適用的TVSDK 1.4至2.5(Java)移轉指南](https://helpx.adobe.com/primetime/migration-guides/tvsdk-14-25-android.html)
* 如需處理螢幕開啟/關閉的情況，請參閱 `Application_Changes_for_Screen_On_Off.pdf` 檔案。
* 請參閱以下章節的完整說明檔案： [Adobe Primetime學習與支援](https://helpx.adobe.com/support/primetime.html) 頁面。
