...
## Primetime Streaming Server 1.4的新功能（12月發行） {#what-s-new-in-primetime-streaming-server-december-release}
...
## Primetime Streaming Server 1.3.1的新增功能（5月發行） {#what-s-new-in-primetime-streaming-server-may-release}
...
### 1.3.1版包含的錯誤修正 {#bug-fixes-included-in-release-1}
...
1. 從 [Oracle站點下載Java SE和JDK軟體](https://www.oracle.com/technetwork/java/javase/downloads/index.html) ，並按照安裝說明操作。
2. 將Adobe Primetime-Streaming Server 1.4封存檔解壓縮 `Primetime- StreamingServer-1-4-0-b206-12042014.zip` 至磁碟。
...
### 安裝和設定Live Packager {#install-and-configure-the-live-packager}

**安裝Live Packager**

1. 從 [Oracle站點下載Java SE和JDK軟體](https://www.oracle.com/technetwork/java/javase/downloads/index.html) ，並按照安裝說明操作。
1. 將Adobe Primetime - Live Packager 1.4封存檔解壓縮 `Primetime-LivePackager-1-4-0-b206-12042014.zip` 至磁碟。

**安裝HTTP原始伺服器**

1. 從 [Oracle站點下載Java JRE和JDK軟體](https://www.oracle.com/technetwork/java/javase/downloads/index.html) ，並按照安裝說明操作。
1. 將Adobe Primetime - HTTP Origin Server 1.4封存檔案解 `Primetime-HttpOrigin-1-4-0-b206-12042014.zip`壓縮至磁碟。

**要啟動Live Packager** To start the packager，請從Packager的根目錄執行以下命令：\
`$packager_start.sh`
...
擷取Adobe Primetime - Live Packager 1.4封存後，將目錄變更為Packager目錄，然後執行Packager_start.sh指令碼。 示例配置偵聽多點傳播地址239.235.0.3:14000，並在埠8080上運行本地源伺服器。 將輸出配置為寫入 `packager/webroot/_default_/_default_/ directory`。
...
### 安裝和配置Offline Packager {#install-and-configure-offline-packager}
...
1. 將Adobe Primetime - Offline Packager 1.4封存檔案解 `Primetime- OfflinePackager-1-4-0-b206-12042014.zip`壓縮至磁碟。

請參閱Primetime Offline Packager快速入門檔案，以取得此處的設定詳細 [資訊](https://help.adobe.com/en_US/primetime/api/packagers/offline/index.html)。
