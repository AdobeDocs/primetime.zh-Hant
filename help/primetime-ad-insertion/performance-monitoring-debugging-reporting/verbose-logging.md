...
* `ptdebug=true`
除了TRACE_HTTP_HEADER和大多數呼叫/回應資料之外，所有記錄都來自TRACE_AD_CALL記錄。

* `ptdebug=AdCall`
只有TRACE_AD_型別(例如TRACE_AD_CALL)記錄。

* `ptdebug=Header`
僅限TRACE_HTTP_HEADER記錄。

## 記錄格式 {#log-record-formats}

每個記錄檔記錄都有一個型別和一組欄位，有些可能是選擇性的。 記錄型別之前的所有記錄的欄位都是相同的。 它們會提供作業階段的時間戳記和資訊。 記錄型別可識別所記錄的事件的型別，後續欄位則提供所記錄事件的相關資訊。
...
| zone_id | 整數 | 區域 |
| record_type | 字串 | 正在記錄的事件型別 |
| 其他欄位 | 因情況而異 | 視事件型別而定 |

此型別的記錄會記錄HTTP請求的結果。 欄位超出 `TRACE_REQUEST_INFO` 會以表格中顯示的順序顯示，並以索引標籤分隔。
...
在資訊清單伺服器與使用者端、廣告伺服器或內容伺服器之間的HTTP呼叫期間，這種型別的記錄會記錄HTTP標頭。 TRACE_HTTP_HEADER以外的欄位會以表格中的順序顯示，並以索引標籤分隔。
...
此型別的記錄記錄會記錄資訊清單伺服器和要求的結果。 欄位超出 `TRACE_AD_CALL` 會以表格中顯示的順序顯示，並以索引標籤分隔。
...
| avail_id | 字串 | 從內容資訊清單檔案中的廣告提示取得之可用的ID （VOD不適用） |
| avail_duration | 數字 | 內容資訊清單檔案中的廣告提示的可用持續時間（秒） （VOD不適用） |
...
此型別的記錄會記錄該記錄型別所指示的廣告請求結果。 記錄型別以外的欄位會依表格中顯示的順序顯示（以索引標籤分隔）。
...
| ad_content_url | 字串 | 廣告伺服器回應中的廣告資訊清單檔案URL。 |
| **†** ad_content_url_actual | 字串 | 插入廣告資訊清單檔案的URL。 留空表示TRACE_AD_REDIRECT。 |
| ad_system_id | 字串 | 廣告系統，來自廣告伺服器回應(若未指定Auditude)。 |
...
>[!NOTE]
>
>的 `TRACE_AD_RESOLVE` 和 `TRACE_AD_INSERT`，此網址位於 `ad_content_url_actual` 欄位適用於轉碼廣告（如有）。 否則欄位會空白 `TRACE_AD_RESOLVE` 或與 `ad_content_url` 的 `TRACE_AD_INSERT`.
...
此型別的記錄會記錄資訊清單伺服器傳送給CRS的轉碼請求結果。 欄位超出 `TRACE_TRANSCODING_REQUESTED` 會以表格中顯示的順序顯示，並以索引標籤分隔。
...
此型別的記錄表示執行伺服器端追蹤的請求。 欄位超出 `TRACE_TRACKING_REQUEST` 會以表格中顯示的順序顯示，並以索引標籤分隔。

| 欄位 | 型別 | 說明 |
|---|---|---|
| tracking_url_count | 整數 | 追蹤URL的數量 |
| 開始 | 浮點數 | PTS片段開始時間（以毫秒為精確度的秒） |
| 結束 | 浮點數 | PTS片段結束時間（以毫秒為精確度的秒） |

此型別的記錄會提供伺服器端追蹤的追蹤URL。 欄位超出 `TRACE_TRACKING_REQUEST_URL` 會以表格中顯示的順序顯示，並以索引標籤分隔。

| 欄位 | 型別 | 說明 |
|---|---|---|
| timestamp | 浮點數 | 播放工作階段內ping追蹤URL的時間（秒，精確度為。001）。 |
| ad_system | 字串 | 廣告系統（例如auditude） |
| url | 字串 | ping的URL |

資訊清單伺服器針對此型別的記錄檔要求進行記錄 `WEBVTT` 註解。 欄位超出 `TRACE_WEBVTT_REQUEST` 會以表格中顯示的順序顯示，並以索引標籤分隔。

| 欄位 | 型別 | 說明 |
|---|---|---|
| 狀態 | 字串 | 已傳回HTTP狀態碼 |
| vtt_uri | 字串 | 請求的URL |
| 開始 | 浮點數 | 分割開始時間（以毫秒為精確度的秒） |
| 結束 | 浮點數 | 分割結束時間（以毫秒為精確度的秒） |

資訊清單伺服器傳送至使用者端的此類記錄回應 `answer` 至以下專案的請求 `WEBVTT` 註解。 欄位超出 `TRACE_WEBVTT_RESPONSE` 會以表格中顯示的順序顯示，並以索引標籤分隔。
...
此型別的記錄會回應資訊清單伺服器的要求 `WEBVTT` 註解。 欄位超出 `TRACE_WEBVTT_SOURCE` 會以表格中顯示的順序顯示，並以索引標籤分隔。

| 欄位 | 型別 | 說明 |
|---|---|---|
| 狀態 | 字串 | 已傳回HTTP狀態碼 |
| 來源 | 字串 | Base64編碼原始VTT內容 |

此型別的記錄可讓資訊清單伺服器記錄事件和資訊，這些資訊不是為擷取廣告而計畫的。 超出欄位 `TRACE_MISC` 由訊息字串組成。 可能顯示的訊息包括：
...
* 忽略提供無效時間值的可用性： *時間*若為availId =可用性ID。
* 忽略提供無效期間值的可用性： *duration *若為availId =可用性ID。
* 初始化新工作階段。 （變體）
* 無效的HTTP方法。 它必須是GET。 (VOD)
* 無效的HTTP方法。 追蹤要求必須為GET。 （即時）
* 無效的URL請求的URL錯誤訊息。 （變體）
...
* 進行廣告呼叫：廣告呼叫URL。
* 找不到：內容URL的資訊清單。 （即時）
* 找不到與可用ID相符的可用：可用ID。 (HLSManifestResolver)
...
* 請求： URL。
* 因為找不到播放工作階段，所以傳回GET要求的錯誤回應。 (VOD)
* 因為內部伺服器錯誤而傳回GET要求的錯誤回應。
...
* 正在從快取傳回資訊清單。 (VOD)
* 伺服器超載。 繼續而不需廣告拼接請求。 （變體）
* 開始產生目標資訊清單。 (HLSManifestResolver)
* 開始從下列來源產生變體資訊清單：內容URL。 （變體）
* 開始將廣告拼接至資訊清單。 (VODHLSResolver)
* 嘗試將廣告拼接在 `HH:MM:SS`： AdPlacement \[adManifestURL=廣告資訊清單URL， durationSeconds=秒， ignore=忽略， redirectAd=重新導向廣告， priority=優先順序。\] \(HLSManifestResolver\)
* 因為無效的時程表而無法取得廣告 — 傳回了不含廣告的內容。 (VOD)
...
資訊清單伺服器會在伺服器端追蹤工作流程期間收到有關播放進度的訊號時，產生此類記錄。 欄位超出 `TRACE_PLAYBACK_PROGRESS` 會以表格中顯示的順序顯示，並以索引標籤分隔。
...
## 多個位元速率串流 {#multiple-bitrate-streams}

廣告插入的使用者端請求通常會在變體M3U8格式的播放清單中指定超過一個位元速率。 資訊清單伺服器會產生並傳回新的變體M3U8檔案，其中包含每個位元速率的個別M3U8連結。 它也會產生唯一的群組ID，以將這些M3U8繫結在一起。

資訊清單伺服器使用使用者端BootstrapURL請求中的資訊來擷取內容變體播放清單。 它會產生新的變體播放清單，包含資料流層級內容的資訊清單伺服器連結。 使用者端會使用這些來建構廣告插入請求。 資訊清單伺服器的串流層級內容連結格式如下：
...
* **live/vod**
資訊清單伺服器會根據內容的播放清單型別設定此值：即時/線性(`#EXT-X-PLAYLIST-TYPE:EVENT`)或VOD (`#EXT-X-PLAYLIST-TYPE:VOD`)

* **publisherAssetId**
BootstrapURL要求中提供之特定內容的發行者唯一ID。

* **轉譯**
資訊清單伺服器會根據 `BANDWIDTH` 內容資料流的值，並用來比對廣告的位元速率與內容的位元速率。 廣告位元速率不能超過內容的位元速率，除非具有最低位元速率的廣告轉譯超過此速率。

* **groupID**
資訊清單伺服器會產生此值，並使用它來確保無論使用者端要求廣告的位元速率轉譯，都能一致地放置廣告。

* **位元速率資料流的base64編碼url**
資訊清單伺服器URL安全base64會編碼內容資料流的絕對URL。 每個資料流都有自己的URL。
